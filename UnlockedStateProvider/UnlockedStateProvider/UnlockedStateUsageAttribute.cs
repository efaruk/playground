...
namespace UnlockedStateProvider
{
    ...
    ///     if disabled, not gets session objects from store on request begin and not push session objects to store on request
    ///     end.
    ...
    public abstract class UnlockedStateUsageAttribute : ActionFilterAttribute
    {
        ...
        protected abstract IUnlockedStateStore UnlockedStateStore { get; }
