...
namespace UnlockedStateProvider.Redis
{
    public class RedisUnlockedStateStore : IUnlockedStateStore
    {
        ...
        public UnlockedStateStoreConfiguration Configuration
        {
        ...
        protected void SlideInternal(string prefix, TimeSpan expireTime, bool async = false)
        {
            ...
            string script =
                string.Format(
                    "local k=0;local karr=redis.call('KEYS', '{0}*'); for i, name in ipairs(karr) do redis.call('EXPIRE', name, {1}); k=i; end return k;)",
                    prefix, Math.Ceiling(expireTime.TotalSeconds));
            Eval(script, async);
        }
        ...
        public void DeleteStartsWith(string prefix, bool async = false)
        {
            ...
            string script =
                string.Format(
                    "local karr=redis.call('KEYS', '{0}*'); for i, name in ipairs(karr) do redis.call('del', name); end",
                    prefix);
            Eval(script, async);
        }
